...
public class RideSharingAlgorithm extends TripSchedulingAlgorithm implements TripSchedulingAlgorithm.algorithm {
    ...
    public static int TripSchedule(Car_list car_list, Customer_demand demand,int trip_index, int max_waiting_time){
        ...
        if(Trip_to_be_shared!=-1){
            System.out.println("Shared once");
            trip_temp=demand.trip_list.get(Trip_to_be_shared);
            trip.setTimestamp_picked_up(trip_temp.getTimestamp_picked_up()-trip.getStartTime()+calculate_distance(trip_temp.getSrc(),trip.getSrc()));
            ...
            distance_from_A_to_C_passing_B=calculate_distance(trip_temp.getSrc(),trip.getSrc(),trip_temp.getDest());
           double distance_pick_up_A=trip_temp.getWaiting_time()*car.speed;
